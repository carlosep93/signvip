...
def render_hands(renderer, result, scaled_focal_length=10000.0):
    all_verts = []
    all_cam_t = []
    all_right = []
    ...
    if len(all_verts) > 0:
        ...
        return Image.fromarray(
            np.clip(255 * input_img_overlay, 0, 255).astype(np.uint8)
        )
    else:
        # return all black image
...
class Renderer:
    ...
    def __init__(self, focal_length, img_res, faces: np.array):
        ...
        Args:
            cfg (CfgNode): Model config file. cfg.EXTRA.FOCAL_LENGTH cfg.MODEL.IMAGE_SIZE
        ...
        mesh_list = [
            pyrender.Mesh.from_trimesh(
                self.vertices_to_trimesh(
                    vvv, ttt.copy(), mesh_base_color, rot_axis, rot_angle, is_right=sss
                )
            )
